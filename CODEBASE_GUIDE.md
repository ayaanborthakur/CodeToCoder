...
- **Pyodide Loading**: It does _not_ load Pyodide here. Pyodide is loaded dynamically in `pyodideService.ts` to improve initial page load speed.
...
### Auth & Pyodide Architecture Choices

### User Authentication

- **Firebase Auth**: Manages user identity.
- **Provider**: Google Sign-In via `signInWithPopup`.
- **Note on Architecture**: We deliberately **DO NOT** use Cross-Origin-Opener-Policy (COOP) headers. This allows `signInWithPopup` to function correctly.
- **Persistence**: Managed by Firebase SDK (typically IndexedDB).

### Python Execution (Pyodide)

- **Environment**: Runs in a Web Worker (`pyodide.worker.ts`).
- **Input Handling (`input()` function)**:
  - We cannot use `SharedArrayBuffer` + `Atomics.wait` because that requires COOP headers, which would break Auth.
  - **Solution**: We use a **Service Worker Bridge** (`input-serviceworker.js`).
  - Flow:
    1. Python calls `input()`.
    2. Worker makes a **Synchronous XHR** request to `/pyodide-input`. This pauses the Worker.
    3. Service Worker intercepts this request and messages the Main Window.
    4. Main Window shows Terminal Input UI.
    5. User typed input is sent back to Service Worker.
    6. Service Worker resolves XHR request.
    7. Python resumes.
  - **Crucial**: This allows "blocking" input in the background while keeping the Main Thread responsive and Auth headers standard.

### Infrastructure Services

- **Gemini Service**: Handles AI interactions for chat and hints.
- **Service Workers**:
  - `input-serviceworker.js`: Handles Pyodide input bridging.
